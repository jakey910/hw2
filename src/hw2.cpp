...
void leadertoworld2D(geometry_msgs::Point &follower_goal, turtlesim::Pose &leader)
{
	...
	*/
	...
	follower_goal.x = temp_x*cos(leader.theta) - temp_y*sin(leader.theta) + leader.x;
	follower_goal.y = temp_x*sin(leader.theta) + temp_y*cos(leader.theta) + leader.y;
} 
...
void Positioncontrol(geometry_msgs::Point &goal, turtlesim::Pose &follower, geometry_msgs::Twist &vel_msg) {
	...
	-----------------------------*/
	float kp=1;
	vel_msg.linear.x = kp*error_norm;
	vel_msg.angular.z = 2*kp*error_theta;
	if (error_norm < 0.1) {
		vel_msg.linear.x = 0;
		vel_msg.angular.z = 0;
	}
